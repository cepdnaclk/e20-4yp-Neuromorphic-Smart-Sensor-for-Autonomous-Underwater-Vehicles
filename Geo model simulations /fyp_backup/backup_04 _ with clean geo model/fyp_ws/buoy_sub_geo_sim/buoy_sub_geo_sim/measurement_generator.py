...
class MeasurementGenerator(Node):
    ...
    def publish_measurements(self):
        ...
        # Convert buoy local pose to RTK lat/lon
        buoy_lat, buoy_lon = xy_to_latlon(self.origin_lat, self.origin_lon, bx, by)

        # add RTK noise in meters
        ...
        # Synthetic tether angles from truth, then noise
        # alpha/beta use east-west geometry, mu/eta use north-south geometry
        ...
        L_meas = max(0.05, min(5.0, L_true + random.gauss(0.0, self.length_noise_m)))
        depth_meas = max(0.01, depth + random.gauss(0.0, self.depth_noise_m))
